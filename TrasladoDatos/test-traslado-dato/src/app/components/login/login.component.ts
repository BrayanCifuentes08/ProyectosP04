...
export default class LoginComponent {
  ...
  onAplicacionSeleccionada(application: any): void {
    ...
    // Buscar displays asociados a la aplicación seleccionada
    this.buscarUserDisplay2(application.application);
  
    // Verificar si la aplicación no tiene displays
    if (!this.userDisplaysPadres || this.userDisplaysPadres.length === 0) {
      console.warn(`La aplicación '${application.name}' no tiene displays asociados.`);
    }
  }
  ...
  validarSeleccion(): boolean {
    ...
    // Si la aplicación seleccionada no tiene displays, la validación se completa aquí
    if (!this.userDisplaysPadres || this.userDisplaysPadres.length === 0) {
      return validacionesBase;
    }
  
    // Función recursiva para verificar si hemos llegado al último nivel de displays
    const esUltimoNivel = (nodo: any): boolean => {
      if (nodo.hijos && nodo.hijos.length > 0) {
        return nodo.hijos.some((hijo: any )=> esUltimoNivel(hijo));
      } else {
        return nodo.display_URL_Alter !== null; // Validar que el display sea válido
      }
    };
  
    // Verificar si la selección de displays es válida
    const seleccionDeDisplayValida = displaysSeleccionados ? 
      this.userDisplaysPadres.some(padre => padre.hijos.some((hijo: any) => esUltimoNivel(hijo))) : false;
  
    // Retornar validación general
    return validacionesBase && seleccionDeDisplayValida;
  }
