...
namespace ApiCanalesDistribucionMTTO.Controllers
{
    ...
    [ApiController]
    public class PaBscCanalDistribucionCtrl : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public PaBscCanalDistribucionCtrl(IConfiguration configuration)
        {
            ...
            [FromQuery] string? pUserName)
        {
            try
            {
                ...
                using (IDbConnection db = new SqlConnection(connectionString))
                {
                    ...
                    if (accion == 1) // SELECT
                    {
                        var resultados = db.Query<PaBscCanalDistribucionM>("Pa_bsc_CanalDistribucion", parameters, commandType: CommandType.StoredProcedure);
                    ...
                    {
                        var resultado = db.QueryFirstOrDefault<PaBscCanalDistribucionM>("Pa_bsc_CanalDistribucion", parameters, commandType: CommandType.StoredProcedure);
                        if (resultado != null)
                        {
