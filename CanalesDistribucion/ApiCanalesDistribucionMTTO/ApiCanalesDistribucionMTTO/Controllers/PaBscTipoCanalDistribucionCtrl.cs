using ApiCanalesDistribucionMTTO.Connection;
using ApiCanalesDistribucionMTTO.Model;
using Dapper;
...
namespace ApiCanalesDistribucionMTTO.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class PaBscTipoCanalDistribucionCtrl : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public PaBscTipoCanalDistribucionCtrl(IConfiguration configuration)
        {
            ...
            [FromQuery] string? pUserName)
        {
            try
            {
                ...
                using (IDbConnection db = new SqlConnection(connectionString))
                {
                    ...
                    if (accion == 1) // SELECT
                    {
                        var resultados = db.Query<PaBscCanalDistribucionM>("Pa_bsc_TipoCanalDistribucion", parameters, commandType: CommandType.StoredProcedure);
                    ...
                    {
                        var resultado = db.QueryFirstOrDefault<PaBscCanalDistribucionM>("Pa_bsc_TipoCanalDistribucion", parameters, commandType: CommandType.StoredProcedure);
                        if (resultado != null)
                        {
