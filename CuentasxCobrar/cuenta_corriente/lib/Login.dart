...
import 'package:test_cuenta_corriente/Models/PaBscApplication1M.dart';
import 'package:test_cuenta_corriente/Models/PaBscEmpresa1M.dart';
import 'package:test_cuenta_corriente/Models/PaBscEstacionTrabajo2M.dart';
import 'package:test_cuenta_corriente/Models/PaBscUser2M.dart';
import 'package:test_cuenta_corriente/Models/PaBscUserDisplay2M.dart';
...
class _LoginScreenState extends State<LoginScreen>
  ...
  String baseUrl = 'http://192.168.1.25/ApiCuentaCorriente/api/';
  String token = "";
  ...
  void _verificarCampos() {
    if (_userController.text == "") {
      Mensajes.mensajeAdvertencia(
          context,
          S.of(context).debeIngresarElUsuario,
          temaClaro ? Colors.white : Color.fromARGB(255, 73, 73, 73),
          temaClaro ? Color.fromARGB(255, 83, 83, 83) : Colors.white,
          temaClaro ? Color.fromARGB(255, 0, 0, 0) : Colors.white);
    ...
    if (_passwordController.text == "") {
      Mensajes.mensajeAdvertencia(
          context,
          S.of(context).debeIngresarLaContrasena,
          temaClaro ? Colors.white : Color.fromARGB(255, 73, 73, 73),
          temaClaro ? Color.fromARGB(255, 83, 83, 83) : Colors.white,
          temaClaro ? Color.fromARGB(255, 0, 0, 0) : Colors.white);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          //Fondo con curvas
          if (widget.isBackgroundSet == false)
            CustomPaint(
              size: Size.infinite,
              painter: Fondo(temaClaro: temaClaro),
            ),
          // Fondo animado con AnimatedBackground
          if (widget.isBackgroundSet == false)
            AnimatedBackground(
              behaviour: RacingLinesBehaviour(numLines: 7),
              vsync: this,
              child: Container(),
            ),
                                  ...
                                  children: [
                                    ...
                                    TextField(
                                      ...
                                      decoration: InputDecoration(
                                        ...
                                        contentPadding: EdgeInsets.symmetric(
                                            vertical: 16, horizontal: 12),
                                        hintText:
                                            '${S.of(context).ingresaContrasena}',
                                        hintStyle:
                                            TextStyle(color: Colors.grey[500]),
                                        prefixIcon: Icon(
                                          Icons.lock,
                                    ...
                                    if (_user2.isNotEmpty &&
                                      ...
                                      DropdownButtonFormField<PaBscEmpresa1M>(
                                        ...
                                        hint: Text(
                                          "${S.of(context).seleccionarEmpresa}",
                                          style: TextStyle(
                                            fontSize: 14,
                                            color: temaClaro
                                                ? Colors.grey.shade600
                                    ...
                                    if (_user2.isNotEmpty &&
                                      ...
                                      DropdownButtonFormField<
                                        ...
                                        hint: Text(
                                          "${S.of(context).seleccionarAplicacion}",
                                          style: TextStyle(
                                            fontSize: 14,
                                            color: temaClaro
                                                ? Colors.grey.shade600
                                    ...
                                    _user2.isNotEmpty &&
                                        ...
                                        ? DropdownButtonFormField<
                                            ...
                                            hint: Text(
                                              "${S.of(context).seleccionarDisplay}",
                                              style: TextStyle(
                                                fontSize: 14,
                                                color: temaClaro
                                                    ? Colors.grey.shade600
                                          ...
                                          onPressed: () {
                                            Navigator.push(
                                              ...
                                              customPageRoute(
                                                ...
                                                Duration(
                                                    milliseconds:
                                                        600), // Duración total de la transición
                                              ),
                                            );
                                          },
                                          child: Text(
...
//DISEÑO DEL FONDO
class Fondo extends CustomPainter {
  final bool temaClaro;

  Fondo({required this.temaClaro});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Fondo principal (dependiendo del tema)
    path.addRect(Rect.fromLTRB(0, 0, size.width, size.height));
    paint.color = temaClaro
        ? Color(0xFF1A2A3A) // Color azul oscuro para modo claro
        : Color(0xFF0A1A2A); // Color más oscuro para modo oscuro
    canvas.drawPath(path, paint);

    // Primera curva (capa inferior)
    path = Path();
    ...
    paint.color = temaClaro
        ? Color.fromARGB(255, 54, 99, 197) // Azul oscuro para modo claro
        : Color.fromARGB(255, 30, 60, 120); // Azul más oscuro para modo oscuro
    canvas.drawPath(path, paint);

    // Nueva curva (capa intermedia)
    path = Path();
    path.moveTo(0, size.height * 0.675);
    path.quadraticBezierTo(
      size.width * 0.5,
      size.height * 0.8,
      size.width,
    ...
    paint.color = temaClaro
        ? Color.fromARGB(255, 29, 142, 247) // Azul claro para modo claro
        : Color.fromARGB(255, 15, 75, 150); // Azul más oscuro para modo oscuro
    canvas.drawPath(path, paint);

    // Tercera curva (capa superior)
    path = Path();
    ...
    paint.color = temaClaro
        ? Colors.white // Blanco para modo claro
        : Color(0xFFE0E0E0); // Gris claro para modo oscuro
    canvas.drawPath(path, paint);
  }
